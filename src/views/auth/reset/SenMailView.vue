<template>
  ...
  >
    <div class="flex flex-col gap-3 items-center w-full">
      <IconComponent icon="sendMailPageIcon" />
      <h2 class="font-semibold text-[1.5rem]">Forgot Password?</h2>
...
const onSubmit = handleSubmit((values: any) => {
  mutate(values, {
    onSuccess: (returnedData: any) => {
      toast({ title: 'Sent', description: returnedData.message })
      if (returnedData.redirect === true) {
        resetPasswordStore.setResetter(returnedData)
